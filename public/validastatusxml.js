			...
			reader.onload = async (e) => {
				...
				try {
					...
					const jsonResult = JSON.parse(
						convert.xml2json(xmlContent, { compact: true, spaces: 4 }),
					);

					// 2. Extraer datos del JSON (AJUSTA ESTO A TU ESTRUCTURA DE JSON)
					const rfcEmisor = jsonResult.Comprobante.Emisor._attributes.Rfc;
					const rfcReceptor = jsonResult.Comprobante.Receptor._attributes.Rfc;
					const total = jsonResult.Comprobante._attributes.Total;
					const folioFiscal =
						jsonResult.Comprobante.Complemento.TimbreFiscalDigital._attributes
							.UUID;
					...
					resultadoDiv.innerHTML =
						"<p>Error al procesar el XML: Formato inválido o datos faltantes</p>";
				}
			};
